...
class Xpack {
  ...
  async downloadBinaries (packagePath, cacheFolderPath) {
    ...
    if (!cacheInfo) {
      ...
      try {
        ...
        const digest = await this.cacheArchive(fileUrl, cacheFolderPath,
          cacheKey, opts)
        log.trace(`digest ${digest} for ${fileUrl}`)
      } catch (err) {
        log.trace(err)
  ...
  async cacheArchive (url, cacheFolderPath, key, opts) {
    ...
    log.debug(`fetch.status ${res.status} ${url}`)
    log.trace(`fetch.statusText ${res.statusText} ${url}`)
    ...
    if (res.ok) {
      if (cacacheUseStream) {
        return new Promise((resolve, reject) => {
          ...
          log.trace(`create pipe for ${key}`)
          res.body.pipe(cacache.put.stream(cacheFolderPath, key, opts)
            .on('integrity', (value) => {
              log.debug(`computed integrity ${value} for ${key}`)
              resolve(value)
            })
            .on('error', (err) => {
              log.trace(`cacache.put.stream error ${err} for ${key}`)
              reject(err)
            })
          ).on('close', () => {
            log.trace(`cacheArchive pipe closef or ${key}`)
            resolve()
          }).on('error', (err) => {
            log.trace(`cacheArchive pipe error ${err} for ${key}`)
            reject(err)
          })
        })
      } else {
        ...
        log.trace(`buffer for ${url} ${data.length} bytes`)
        return await cacache.put(cacheFolderPath, key, data, opts)
      }
    }
