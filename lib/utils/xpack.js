...
export class Xpack {
  ...
  async downloadBinaries ({ packagePath, packageTmpPath, cacheFolderPath }) {
    const context = this.context
    const config = context.config
    const log = this.log
    ...
    if (!cacheInfo) {
      ...
      try {
        await this.cacheArchive(fileUrl, cacheFolderPath, cacheKey, opts)
    ...
    if (log.isVerbose()) {
      ...
      log.info(`${res.length} files => '${contentFolderPath}'`)
    }
  }
  ...
  isBinaryXpack (json = this.packageJson) {
    ...
    return (
      !!json && !!json.xpack && (!!json.xpack.executables || !!json.xpack.bin)
    )
  }
  ...
  parsePackageSpecifier ({ packSpec }) {
    assert(packSpec)
    ...
    if (packSpec.startsWith('@')) {
      ...
      if (arr.length > 2) {
        throw new CliError(`'${packSpec}' not a package name`)
      }
      scope = arr[0]
      if (arr.length > 1) {
  ...
  retrieveConfiguration ({ packageJson, configurationName }) {
    ...
    if (
      !packageJson.xpack.configurations &&
  ...
  async computeBuildFolderRelativePath ({
    ...
    if (buildFolderRelativePath) {
      ...
      log.warn(
        'neither "configuration.properties.buildFolderRelativePath" ' +
          'nor "xpack.properties.buildFolderRelativePath" were found in ' +
          'package.json, using default ' +
          `"${buildFolderRelativePath}"...`
      )
    }
  ...
  async checkMinimumXpmRequired (packageJson) {
    ...
    if (!packageJson.xpack || !packageJson.xpack.minimumXpmRequired) {
    ...
    if (semver.lt(xpmVersion, minimumXpmRequired)) {
      throw new CliError(
        ...
        CliExitCodes.ERROR.PREREQUISITES
      )
    }
    // Check passed.
