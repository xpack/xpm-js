...
export class Init extends CliCommand {
  ...
  async doInitWithTemplate () {
    ...
    if (!globalJson) {
      ...
      await globalPackage.pacoteExtract({
        specifier: config.template,
      ...
      try {
        await globalPackage.checkMinimumXpmRequired(context.rootPath)
      } catch (err) {
      ...
      if (
        globalJson.bundleDependencies === undefined &&
