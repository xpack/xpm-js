...
export class Uninstall extends CliCommand {
  ...
  async doRun (args) {
    ...
    let minVersion
    try {
      minVersion = await xpmPackage.checkMinimumXpmRequired(context.rootPath)
    } catch (err) {
    ...
    if (this.jsonPackage && config.mustRewritePackageJson) {
      await xpmPackage.rewritePackageJson(this.jsonPackage)
    }
    ...
    if (stat) {
      if (stat.isDirectory()) {
        if (!config.isDryRun) {
          ...
          log.info(
            `Folder '${dependencyShownLocation}'` + ' should be removed'
          )
        }
  ...
  updateDependencies (key) {
    ...
    if (!config.doNotSave) {
      ...
      if (configurationName) {
        ...
        target = this.buildConfiguration.jsonBuildConfiguration
      } else {
        if (this.policies.shareNpmDependencies) {
          target = packageJson
        } else {
      ...
      if (target.dependencies && target.dependencies[key]) {
        if (!config.isDryRun) {
          ...
          log.verbose(
            `Pretending to remove dependencies['${key}'] ` + 'from package.json'
          )
        }
      }
  ...
  async removeDotBinLinks ({
    ...
    const xpmPackage = new XpmPackage({ log, folderPath: packagePath })
