...
export class Install extends CliCommand {
  ...
  async collectDependency ({
    ...
    try {
      ...
      if (isString(value)) {
        ...
        version = value.specifier.replace(/^[^0-9]+/, '')
        if (isString(value.local) && ['link', 'copy'].includes(value.local)) {
          dependencyKind = value.local
        } else {
        ...
        log.warn(`dependency '${key}' has an unsupported 'specifier' ` +
          'property, ignored')
        return
      }
      pack = `${key}@${version}`
  ...
  async pacoteExtractPackage ({
    ...
    log.trace(
      ...
      `pacoteExtractContent('${fromFolderPath}')`)
