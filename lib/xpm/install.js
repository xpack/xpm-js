...
export class Install extends CliCommand {
  ...
  async installPackage (pack) {
    ...
    if (config.isSystem) {
      // System install.
      await this.installPackageInSystem({ pack, cacheFolderPath, manifestIds })
  ...
  async installPackageInXpack ({
    ...
    if (config.configurationName) {
      ...
      let liquidMap
      try {
        liquidMap = liquidEngine.prepareMap(
          packageJson,
  ...
  async collectConfigurationDependencies ({
    ...
    let liquidMap
    try {
      liquidMap = liquidEngine.prepareMap(packageJson, configurationName)
    } catch (err) {
      log.trace(util.inspect(err))
      throw new CliError(err.message)
    }
   ...
   * @returns {undefined} Nothing.
   */
  async collectDependency ({ key, value, isDev, outputMap }) {
    const log = this.log
  ...
  async pacoteExtractPackage ({
    ...
    const destinationJson = await xpack.isFolderPackage(destinationFolderPath)
    if (destinationJson) {
    ...
    if (config.isDryRun) {
      ...
      log.trace(
        `rename(${destinationTmpFolderPath}, ${destinationFolderPath})`
      )
