...
export class Install extends CliCommand {
  ...
  async installPackageInSystem ({ specifier, cacheFolderPath, manifestIds }) {
    ...
    const log = this.log
    log.trace(
      `${this.constructor.name}.installPackageInSystem('${specifier}')`
    )
  ...
  async installPackageGlobally ({
    ...
    const log = this.log
    log.trace(
      `${this.constructor.name}.installPackageGlobally('${specifier}')`
    )
  ...
  async installPackageInXpmPackage ({
    ...
    if (!jsonGlobal || config.doForce) {
      ...
      await globalXpmPackage.pacoteExtractPackage({
        packFullName,
        specifier: manifestIds.getPacoteFrom(),
        destinationFolderPath: globalPackagePath,
        cacheFolderPath,
    ...
    if (config.doCopy) {
      await destinationXpmPackage.pacoteExtractPackage({
        packFullName,
        specifier: manifestIds.getPacoteFrom(),
          ...
          if (this.policies.shareNpmDependencies) {
            ...
            if (npmPackage.isXpmPackage()) {
              ...
              if (log.isVerbose) {
                ...
                log.info(
                  `+ ${key}@${jsonNpm.version} => 'node_modules/${key}'`
                )
              }
            }
          } else {
