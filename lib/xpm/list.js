...
export class List extends CliCommand {
  ...
  async doRun (args) {
    ...
    this.jsonPackage = await xpmPackage.readPackageDotJsonNoThrow()
  ...
  async listPackagesLocally () {
    ...
    // Only the configurations are used, no need to initialise the actions.
    await xpmLiquidPackage.buildConfigurations.initialise()
    this.xpmLiquidPackage = xpmLiquidPackage

    await xpmLiquidPackage.buildConfigurations.initialise()
    if (buildConfigurationName) {
      if (!xpmLiquidPackage.buildConfigurations.has(buildConfigurationName)) {
        throw new CliErrorInput(
      ...
      const buildConfigurationsNames =
        xpmLiquidPackage.buildConfigurations.names()
      for (const buildConfigurationName of buildConfigurationsNames) {
        const buildConfiguration =
          await xpmLiquidPackage.buildConfigurations.get(
            buildConfigurationName
          )
        // TODO: make them objects too.
        if (
          Object.keys(buildConfiguration.jsonDevDependencies).length > 0 ||
          Object.keys(buildConfiguration.jsonDependencies).length > 0 ||
          log.isVerbose
        ) {
          log.info()
  ...
  async listOneFolderRecursively ({
    ...
    if (stat && stat.isDirectory()) {
      ...
      for (const dirent of dirents) {
        ...
        const subFolderPackage = new XpmPackage({
          log,
          folderPath: subFolderPath
        })

        const jsonSubFolder = await subFolderPackage.readPackageDotJsonThrow()
        if (subFolderPackage.isNpmPackage(jsonSubFolder)) {
      ...
      if (depth === 1) {
        if (hasBin) {
          ...
          for (const binaryDirent of binaryDirents) {
            const tmp =
              `${localFolderName}/` + `${dotBin}/${binaryDirent.name}`
            log.output(`- ${tmp}`)
          }
        }
      }
  ...
  async findGlobalXpmPackagesRecursively ({ folderPath, foundPackagesMap }) {
    ...
    const json = await xpmPackage.readPackageDotJsonNoThrow()

    if (json) {
      let foundVersionsMap = foundPackagesMap.get(json.name)
      if (!foundVersionsMap) {
        foundVersionsMap = new Map()
        foundPackagesMap.set(json.name, foundVersionsMap)
      }
      ...
      foundVersionsMap.set(json.version, content)
