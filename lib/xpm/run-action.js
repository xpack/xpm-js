...
export class RunAction extends CliCommand {
  ...
  async executeAction ({ name, args, configurationName }) {
    ...
    const commandsArrayOriginal = commandsArray.slice()
    commandsArray = []

    // Perform liquid substitutions line by line, to prevent multiple
    // liquid assign statements to overwrite each other.
    for (const command of commandsArrayOriginal) {
      try {
        const substituted = await liquidEngine.performSubstitutions(
          command,
          liquidMap
        )
        if (substituted.includes(os.EOL)) {
          commandsArray.push(...substituted.split(os.EOL))
        } else {
          commandsArray.push(substituted)
        }
      } catch (err) {
        log.trace(util.inspect(err))
        throw new CliError(err.message)
    ...
    if (configurationName) {
      log.debug(`${pack.name}@${pack.version} ${configurationName}/${name}`)
    } else {
    ...
    pathsArray =
      await this.prependPathIfPresent(config.cwd, 'node_modules', pathsArray)
    pathsArray =
      await this.prependPathIfPresent(config.cwd, 'xpacks', pathsArray)
    ...
    if (configurationName) {
      ...
      pathsArray =
        await this.prependPathIfPresent(buildPath, 'xpacks', pathsArray)
    }
