...
export class RunAction extends CliCommand {
  ...
  async doRun(args) {
    ...
    if (args.length === 0) {
      ...
      if (
        ...
        for (const buildConfigurationName of xpmLiquidPackage.buildConfigurations.names()) {
  ...
  async showScripts() {
    ...
    if (!xpmLiquidPackage.buildConfigurtions.empty()) {
      for (const buildConfigurationName of xpmLiquidPackage.buildConfigurtions.names()) {
        const buildConfiguration = xpmLiquidPackage.buildConfigurations.get(
          buildConfigurationName
        )
        await buildConfiguration.actions.initialise()
  ...
  async executeAction({ name, args, buildConfigurationName }) {
    ...
    if (buildConfigurationName) {
      ...
      actions = xpmLiquidPackage.topActions
    }
    const commandsArray = await actions.get(name).getCommands()
