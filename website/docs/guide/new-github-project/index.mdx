...
description: Detailed instructions on how to create a new GitHub repository.
...
This definition does not mandate any particular folder hierarchy
or version control system, if any.

Therefore, an xpm package (xPack) can be created using any tool and stored in
any version control system.

:::tip

All µOS++ sub-projects are hosted in public **GitHub** repositories.
For convenience, it is recommended to host new projects on GitHub as well.
...
- preferably rename it:
  - add a prefix with the name of the original organisation
...
Edit the README file to include:
...
- if the package is inspired by other existing code, enter the
  original code's author as a contributor, for example:
...
- `test-ci.yml` (to be executed automatically upon pushes)
...
- `npm version v1.10.0-1.1`; the first four numbers are the same as the
...
In general, depending on the _disruption_ degree, choose one of the following:
