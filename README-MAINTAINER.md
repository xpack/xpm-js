...
`package-lock.json`, build the release with **node 16.x**,
**npm 8.x** (`"lockfileVersion": 2`).
...
Edit `package.json` to this version suffixed by `-pre` (like `0.19.1-pre`).
...
add a new milestone like `0.19.1` (without `v`)
...
- add an entry with the new version _* v0.19.1 released_
- commit with a message like _prepare v0.19.1_
...
- name the file like `2023-09-26-xpm-v0-17-0-released.md`
- name the post like: **xPack xpm v0.19.1 released**
...
- commit with a message like **xPack xpm v0.19.1 release**
- push
...
- `npm dist-tag ls xpm`
- `npm dist-tag add xpm@0.19.1 latest`
- `npm dist-tag ls xpm`
...
In the issues milestones, close 0.19.1:
...
- paste the release name like **xPack xpm v0.19.1 released**
