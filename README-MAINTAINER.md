...
`package-lock.json`, build the release with **node 18.x**,
...
Edit `package.json` to this version suffixed by `-pre` (like `0.20.1-pre`).
...
add a new milestone like `0.20.1` (without `v`)
...
- add an entry with the new version _* v0.20.1 released_
- commit with a message like _prepare v0.20.1_
...
- name the post like: **xPack xpm v0.20.1 released**
...
- commit with a message like **xPack xpm v0.20.1 release**
- push
...
- `npm dist-tag ls xpm`
- `npm dist-tag add xpm@0.20.1 latest`
- `npm dist-tag ls xpm`
...
In the issues milestones, close 0.20.1:
...
- paste the release name like **xPack xpm v0.20.1 released**
